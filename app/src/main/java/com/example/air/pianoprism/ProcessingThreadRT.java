     ...
     */
    public class ProcessingThreadRT extends Thread {
        ...
        public ProcessingThreadRT(Handler updateUIHandler, int bufferSize, double res, double[][] nmat, int fs) {
        ...
        Handler processHandle = new Handler() {
            public void handleMessage(Message msg) {
                ...
                col1.assign(hund, Functions.mult);
                col1.assign(round);
                col1.assign(hund, Functions.div);

                //  round(nmat (:, 2)*100)/100
                col2.assign(hund, Functions.mult);
                col2.assign(round);
                col2.assign(hund, Functions.mult);

                double minBeat = col1.aggregate(Functions.less, Functions.identity);
                double maxBeat = col1.copy().assign(col2.copy(), Functions.plus).aggregate(retBigger, Functions.identity);

                // 60*(max(nmat(:,1))-min(nmat(:,1))) /(max(nmat(:,6))-min(nmat(:,6)));
                double scoreTempo = 60 * (col1.copy().aggregate(retBigger, Functions.identity)
                        - col1.copy().aggregate(retSmaller, Functions.identity)) /
                        (nmatDM.viewColumn(6).copy().aggregate(retBigger, Functions.identity)
                                -
                                nmatDM.viewColumn(6).copy().aggregate(retSmaller, Functions.identity)
                        );
                ...
                if (cont1) {
                            ...
                            )
                    {
                        ...
                        if (bSpecSub) {
                            ...
                            specFlux = onsetDet.getSpecFlux();
                            rmsCur = onsetDet.getRms();
                        ...
                        * */
                         ...
                         */
                           ...
                           DenseDoubleMatrix2D res = new DenseDoubleMatrix2D(new double[particles.length][particles[0].length]);
                           DenseDoubleMatrix2D Fd = new DenseDoubleMatrix2D(F);
                           Fd.zMult(px,res);


                            particles = res.toArray();
                            // 1 - <
                            // 2 - >
                            inxs = inxsThatSatisfyComparisonRow(particles, maxBeat, 2).get(0).toArray();
                            inxs_int = new int[inxs.length];
       ...
       //         updateUIHandler.sendMessage(createBundleMsg(new Object()));
