package com.example.air.pianoprism;// Wav file IO class
// A.Greensted
...
public class WavFile
{
	...
	public static WavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		...
		putLE(18,							wavFile.buffer, 4, 4);		// Chunk Data Size -------
		putLE(3,								wavFile.buffer, 8, 2);		// Compression Code (Uncompressed)
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		while (true)
		{
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				if ((compressionCode != 1) || (compressionCode != 3)) throw new WavFileException("Compression Code " + compressionCode + " not supported");
	...
	private double readSample(Object... params) throws IOException, WavFileException
	{
		double val = 0;
